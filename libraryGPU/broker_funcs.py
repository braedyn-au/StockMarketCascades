...
def instantMatch(traderIDs, broker, transactions):
    ...
    # iterate through all broker orders
        print(broker.iloc[sID])
        Vol = (broker.iloc[sID]['order'][sID]) #selling amount
        ToS = (broker.iloc[sID]['time'][sID])
        stock = (broker.iloc[sID]['stock'][sID])

        # print(Vol, type(Vol))#, Vol[0], type(Vol[0]))
        # print(ToS, type(ToS))#, ToS[0], type(ToS[0]))
        # print(stock, type(stock))#, stock[0], type(stock[0]))

        trader = traderIDs[broker.iloc[sID]['portfolio'][sID]]
