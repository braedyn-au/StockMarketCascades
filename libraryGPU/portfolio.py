...
import random 
import string
from libraryGPU.utils import sharpe, characterize, sigmoid, config
...
class portfolio:
    ...
    def optimize(self, t = tinit,first=False, window=config.window ):#, stockPool=stockPool):
        ...
        # print("alloc")
        # print(self.alloc.values())
        # print(np.asarray(list(self.alloc.values())))
        # print(np.sum(np.asarray(list(self.alloc.values()))), type(np.sum(np.asarray(list(self.alloc.values())))))
        # print(float(np.sum(np.asarray(list(self.alloc.values())))))
        try:
            assert round(float(np.sum(np.asarray(list(self.alloc.values()))))) == 1
        except:
            print('/n')
        ...
        opt= (minimize(sharpe, 
                      ...
                      args=(stockPool,self.stocks,float(self.value[-1]),ti,t), 
                      method='SLSQP', 
                      bounds=bounds,
                      constraints=cons)['x'])
        ...
        if first:
            ...
            self.alloc['cash'] = 1 - sum(list(self.alloc.values())[:-1])
            self.cash = np.concatenate((self.cash, np.asarray([self.alloc['cash']*self.volume])))
        ...
        self.sharpeNonOpt = np.concatenate((self.sharpeNonOpt, np.asarray([-sharpe(self.initAlloc,stockPool,self.stocks,float(self.value[-1]),ti,t)])))
        self.sharpeOpt = np.concatenate((self.sharpeOpt, np.asarray([-sharpe(opt,stockPool,self.stocks,float(self.value[-1]),ti,t)])))
        self.sharpeReal = np.concatenate((self.sharpeReal, np.asarray([-sharpe(list(self.alloc.values()),stockPool,self.stocks,float(self.value[-1]),ti,t)])))
    ...
    def thresholdOrder(self, time, window=config.window):
        ...
        new = -sharpe(opt,stockPool,self.stocks,float(self.value[-1]),ti,time)
    ...
    def order(self, time, opt=None, changePrice = changePrice):#, stockPool = stockPool):
        ...
        optweights = []
        for i,j in enumerate(self.stocks):
            optweights.append(np.floor(opt[i]*float(self.value[-1]/stockPool[j][time])))
        
        self.orders = (np.asarray(optweights) - np.asarray(list(self.weights.values())))
        orderList = pd.DataFrame({'time':numpy.repeat(time,len(self.stocks)), "portfolio":numpy.repeat(self.portfID, len(self.stocks)),"stock":self.stocks, "order": self.orders})
    
        # update weights that have been sent off and portfolio value
        i = 0
        fvalue = 0
        fcash = float(self.cash[-1])
        for stock,weight in self.weights.items():
            self.weights[stock] = weight + self.orders[i]
            if self.orders[i]<0:
                #SELL
                fcash += float(-self.orders[i]*stockPool[stock][time])
            elif self.orders[i]>0:
                #BUY
                fcash += float(-self.orders[i]*stockPool[stock][time])
        
            # Update portfolio net value
            fvalue += self.weights[stock]*float(stockPool[stock][time])

            if changePrice and self.orders[i]!=0:
                priceChange_random(stock,time,volume=float(self.orders[i]))

            i+=1

        fvalue += fcash
        self.value = np.concatenate((self.value,np.asarray([fvalue])))           
        self.cash = np.concatenate((self.cash, np.asarray([fcash])))
        # renormalize alloc based on new portfolio value
        for stock,weight in self.weights.items():
            self.alloc[stock] = float(self.weights[stock]*float(stockPool[stock][time])/self.value[-1]) # i think this works
        
        # get cash percentage by normalizing weight
        self.alloc['cash'] = fcash/float(self.value[-1])
    ...
    def buy(self,stock,time,volume, changePrice=changePrice):
        ...
        ivalue = float(self.value[-1])
        self.value = np.concatenate((self.value, np.asarray([ivalue + volume*float(stockPool[stock][time])])))
        # self.alloc[stock] = self.weights[stock]*float(stockPool[stock][time])/self.value[-1] needs to be done all at once
        if changePrice:
    ...
    def updateWeightData(self, time):
        ...
        for stock, weight in self.weights.items():
            ...
            fvalue += self.weights[stock]*float(stockPool[stock][time])
        fvalue += float(self.cash[-1])
        self.value = np.concatenate((self.value,np.array([fvalue])))
...
def portfGen(stockPool=stockPool, n=config.nportfs, sizeMin=config.minPortfSize, sizeMax=config.maxPortfSize, overlapMin = config.overlapMin, overlapMax=config.overlapMax):
    ...
    """
    stocks = np.arange(stockPool.shape[0]) #check if 0 or 1
    traderIDs = {}
    ...
    for portfs in range(n):
        ...
        window = int(np.random.randint(sizeMin,sizeMax))
        overlap = int(np.random.randint(overlapMin,overlapMax))
        vol = 10**(int(np.random.randint(4,6)))
...
def uniquePortfGen(n=5, availStocks = stockPool.shape[0]):
    """
    no overlap
    """
    traderIDs = {}
...
def priceChange_random(stock, time, volume, proportion = 1000):
    ...
    increment = float(np.random.choice(np.linspace(-volume/proportion,volume/proportion,7), size=1))
