...
def brokerage(traderIDs, time, broker,totalOrders):
    ...
    """
    def brokerageMP(portf, time, broker, totalOrders):
        orderList = portf.order(time=time)
        print('orderlist: ',orderList)
        broker = pd.concat([broker,orderList])
        totalOrders = pd.concat([totalOrders,orderList])
    
    processes = []
    for key, portf in traderIDs.items():
        p = Process(target=brokerageMP, args=(portf,time,broker,totalOrders)#, kwargs={'time':time, 'broker':broker, 'totalOrders':totalOrders})
        processes.append(p)
        p.start()
    for process in processes:
        process.join()
    
    print(broker, totalOrders)
    return broker, totalOrders

def thresholdBrokerage(traderIDs,time,broker,totalOrders):
    """
    runs thresholdOrder
    """
    def thresholdBrokerageMP(portf):
